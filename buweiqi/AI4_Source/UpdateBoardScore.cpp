...
void AI4::AddScore(int row1, int row2, T1 score, T2 ScorePoor)
{
	...
	int level = 0;
	if (Score[row1][row2] > 0) Score[row1][row2] += score;//先对中心加分
	///0——左  1——上  2——右  3——下
	...
	while (score - ScorePoor > 0)
	{
		...
		if (edge[0] == false && edge[1] == true && Score[angelx1][angely1] > 0) Score[angelx1][angely1] += score;
		///在未到达上边缘时，继续为右边缘的一个位置加分/减分
		if (edge[1] == false && edge[2] == true && Score[angelx2][angely2] > 0) Score[angelx2][angely2] += score;
		///在未到达右边缘时，继续为下边缘的一个位置加分/加分
		if (edge[2] == false && edge[3] == true && Score[angelx3][angely3] > 0) Score[angelx3][angely3] += score;
		///在未到达下边缘时，继续为左边缘的一个位置加分/减分
		if (edge[3] == false && edge[0] == true && Score[angelx4][angely4] > 0) Score[angelx4][angely4] += score;
		//当加分区域扩散到左边缘时
